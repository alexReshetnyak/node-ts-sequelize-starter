...
import UserModel, { IUserModel } from './model-mongo';
import UserValidation from './validation-mongo';
import { IUserService } from './interface-mongo';
...
const UserService: IUserService = {
  ...
  async findOne(id: string): Promise<IUserModel> {
    try {
      ...
      return await UserModel.findOne({ _id: Types.ObjectId(id) });
    } catch (error) {
      throw new Error(error.message);
    }
  },
